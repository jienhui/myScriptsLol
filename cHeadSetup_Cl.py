...
# Head Setup Class
class HeadSetup_Cl():
    def __init__( self, neck, head, jaw, lEye, rEye, tongue, upTeeth, dnTeeth, name ):
        self.neck= neck
        self.head= head
        self.jaw= jaw
        self.lEye= lEye
    ...
    # Main head Setup Function
    def cHeadSetup_Fn( self ): 
    
    # IK Neck Setup Function
        self.ikNeckSetup_Fn()
    
    def ikNeckSetup_Fn(self):
        c= CC.CCtrlHrc_Cl()
        cmds.select( self.head, hi=1 )
        headJntSel= cmds.ls( sl=1 )
        headEndPos= cmds.getAttr( '%s.tx' % headJntSel[1] )
        self.neckIK= cmds.ikHandle( n= 'neck01_ikHandle', sj= self.neck, ee= self.head, sol= 'ikRPsolver', p=1, w=1 )
        ...
        cmds.delete( tmpCnst )
        cmds.parentConstraint( self.neckCtrl[0], self.neck, mo=1 )
        cmds.hide( self.neckIK )
        ...
        cmds.addAttr( self.headCtrl, ln='follow', at='enum', en='World:Neck:' )
        cmds.setAttr( '%s.follow' % self.headCtrl, e=1, l=False, k=True, cb=True )
        ...
        cmds.pointConstraint( self.headCtrl, self.neckIK[0], mo=1 )
...
#    abc= HeadSetup_Cl( 'neck01_jnt', 'head01_jnt', 'jaw01_jnt', 'l_eye01_jnt', 'r_eye01_jnt', 'tongue01_jnt', 'upper_teeth01_jnt', 'lower_teeth01_jnt', 'head')
